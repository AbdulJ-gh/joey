...
type IncomingEventHandler = (event: FetchEvent) => Promise<void>;
...
export class Logger {
	...
	private event: FetchEvent = {} as FetchEvent;
	...
	public static event(logger: Logger, event: FetchEvent) {
		logger.event = event;
		event.waitUntil(logger.incomingEventHandler(event));
	}
	...
	private levelLog(logLevel: number, ...args: unknown[]): void {
		if (logLevel >= this.levelIndex) {
			logLevel === 5
				? this.event.waitUntil(this.exceptionHandler(args[5]))
				: this.event.waitUntil(this.logHandler(this.levels[logLevel], ...args));
