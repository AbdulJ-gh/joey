...
export function get(headers: Headers, name: string) {
...
export function getAll(headers: Headers) {
...
export function set(headers: Headers, name: string, value: string, options?: CookieOptions) {
	let cookie = `${encodeURIComponent(name)}=${encodeURIComponent(value)};`;
	if (options) { cookie += mapOptionsToString(options); }
	headers.append('set-cookie', cookie);
}

export function clear(headers: Headers, name: string, options?: Pick<CookieOptions, 'domain' | 'path'>) {
